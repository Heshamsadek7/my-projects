...
def load_data():
    try:
        ...
        for i, col in enumerate(ncr.columns, 1):
            print(f"   {i}. {col}")
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            ...
            # البيانات التفصيلية للحالات المفتوحة
            html.Div([
                html.H3("📋 الحالات المفتوحة - متابعة", 
                       style={'textAlign': 'center', 'marginBottom': '15px', 'color': 'white',
                              'background': '#ff7675', 'padding': '10px', 'borderRadius': '5px'}),
                html.Div(id='open-cases-info', style={'color': 'white', 'textAlign': 'center', 'marginBottom': '10px'}),
...
@app.callback(
    [Output('total-cases', 'children'),
     ...
     Output('open-cases-table', 'columns'),
     Output('open-cases-info', 'children')],
...
def update_dashboard(selected_month, selected_department):
    try:
        ...
        # البيانات التفصيلية للحالات المفتوحة فقط
        open_cases_data = filtered_data[filtered_data['المتابعة\nFollow Up'] == 'Opened']
        
        print(f"🔍 عدد الحالات المفتوحة: {len(open_cases_data)}")
        
        # طريقة مباشرة لاستخراج العمودين P و L - بالترتيب القديم الذي كان يعمل
        try:
            # الحصول على العمودين حسب الموقع - بالترتيب القديم
            department_data = open_cases_data.iloc[:, 15]  # العمود P
            description_data = open_cases_data.iloc[:, 11]  # العمود L
            
            # إنشاء DataFrame جديد بالعمودين المطلوبين
            open_cases_display = pd.DataFrame({
                'الإدارة': department_data,
                'وصف حالة عدم المطابقة': description_data
            })
            
            # تنظيف البيانات - إزالة القيم الفارغة
            open_cases_display = open_cases_display.dropna(subset=['وصف حالة عدم المطابقة'])
            
            columns = [
                {"name": "الإدارة", "id": "الإدارة"},
                {"name": "وصف حالة عدم المطابقة", "id": "وصف حالة عدم المطابقة"}
            ]
            
            info_message = f"عرض {len(open_cases_display)} حالة مفتوحة"
            print(f"✅ تم استخراج {len(open_cases_display)} حالة مفتوحة بنجاح")
            
        except Exception as e:
            print(f"❌ خطأ في استخراج العمودين: {e}")
            open_cases_display = pd.DataFrame({
                'رسالة': ['لا توجد حالات مفتوحة أو حدث خطأ في استخراج البيانات']
            })
            columns = [{"name": "رسالة", "id": "رسالة"}]
            info_message = "❌ حدث خطأ في عرض البيانات"
        ...
        if not filtered_data.empty and 'الإدارة المتواجد بها الحالة' in filtered_data.columns:
            ...
            fig_department = px.bar(title="لا توجد بيانات للإدارات")
            fig_department.update_layout(
                plot_bgcolor='#1e1e1e', 
                paper_bgcolor='#1e1e1e', 
                font_color='white',
        ...
        if not filtered_data.empty and 'إسم الشخص القائم بالتبليغ' in filtered_data.columns:
            ...
            fig_employee = px.bar(title="لا توجد بيانات للموظفين")
            fig_employee.update_layout(
                plot_bgcolor='#1e1e1e', 
                paper_bgcolor='#1e1e1e', 
                font_color='white',
        ...
        if not ncr.empty and 'شهر' in ncr.columns:
            ...
            fig_monthly = px.bar(title="لا توجد بيانات للمقارنة الشهرية")
            fig_monthly.update_layout(
                plot_bgcolor='#1e1e1e', 
                paper_bgcolor='#1e1e1e', 
                font_color='white',
        ...
        return (
            ...
            fig_employee,
            fig_monthly,
            open_cases_display.to_dict('records'),
            columns,
            info_message
        )
        ...
        print(f"Error: {e}")
        # إرجاع رسوم بيانية فارغة في حالة الخطأ
        empty_fig = px.bar(title="حدث خطأ في تحميل البيانات")
        ...
        return (
            [html.H4("0", style={'margin': '0', 'fontSize': '1.5em', 'fontWeight': 'bold'}), 
             html.P("إجمالي الحالات", style={'margin': '0', 'fontSize': '0.9em'})],
...
if __name__ == '__main__':
    ...
    print("📊 افتح المتصفح على: http://localhost:8060")
