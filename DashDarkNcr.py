...
def update_dashboard(selected_month, selected_department):
    try:
        ...
        # 1. رسم الإدارات (كامل العرض مع Closed/Opened)
        if not filtered_data.empty and 'الإدارة المتواجد بها الحالة' in filtered_data.columns:
            dept_data = filtered_data.groupby(['الإدارة المتواجد بها الحالة', 'المتابعة\nFollow Up']).size().unstack(fill_value=0).reset_index()
            
            # التأكد من وجود الأعمدة المطلوبة
            if 'Closed' not in dept_data.columns:
                dept_data['Closed'] = 0
            if 'Opened' not in dept_data.columns:
                dept_data['Opened'] = 0
                
            dept_data['المجموع'] = dept_data['Closed'] + dept_data['Opened']
            dept_data = dept_data.sort_values('المجموع', ascending=False)
            
            fig_department = px.bar(dept_data, x='الإدارة المتواجد بها الحالة', y=['Closed', 'Opened'], 
                                  title="📊 توزيع الحالات حسب الإدارة (مغلقة vs مفتوحة)",
                                  barmode='group',
                                  color_discrete_map={'Closed': '#00b894', 'Opened': '#ff7675'})
            
            # إضافة القيم داخل الأعمدة
            fig_department.update_traces(texttemplate='%{y}', textposition='inside')
        ...
        # 2. رسم الموظفين (عمودي مع Closed/Opened)
        if not filtered_data.empty and 'إسم الشخص القائم بالتبليغ' in filtered_data.columns:
            emp_data = filtered_data.groupby(['إسم الشخص القائم بالتبليغ', 'المتابعة\nFollow Up']).size().unstack(fill_value=0).reset_index()
            
            if 'Closed' not in emp_data.columns:
                emp_data['Closed'] = 0
            if 'Opened' not in emp_data.columns:
                emp_data['Opened'] = 0
                
            emp_data['المجموع'] = emp_data['Closed'] + emp_data['Opened']
            emp_data = emp_data.sort_values('المجموع', ascending=False).head(10)
            
            fig_employee = px.bar(emp_data, x='إسم الشخص القائم بالتبليغ', y=['Closed', 'Opened'],
                                title="👥 أداء الموظفين (مغلقة vs مفتوحة)",
                                barmode='group',
                                color_discrete_map={'Closed': '#00b894', 'Opened': '#ff7675'})
            
            fig_employee.update_traces(texttemplate='%{y}', textposition='inside')
        ...
        # 3. المقارنة الشهرية (بالإدارات و Closed/Opened)
        if not ncr.empty and 'شهر' in ncr.columns:
            # تجميع البيانات الشهرية مع الإدارات
            monthly_dept_data = ncr.groupby([ncr['شهر'].dt.month, 'الإدارة المتواجد بها الحالة', 'المتابعة\nFollow Up']).size().unstack(fill_value=0).reset_index()
            
            if 'Closed' not in monthly_dept_data.columns:
                monthly_dept_data['Closed'] = 0
            if 'Opened' not in monthly_dept_data.columns:
                monthly_dept_data['Opened'] = 0
                
            monthly_dept_data['شهر'] = monthly_dept_data['شهر'].map({
                1: 'يناير', 2: 'فبراير', 3: 'مارس', 4: 'أبريل', 5: 'مايو', 6: 'يونيو',
                7: 'يوليو', 8: 'أغسطس', 9: 'سبتمبر', 10: 'أكتوبر', 11: 'نوفمبر', 12: 'ديسمبر'
            })
            
            fig_monthly = px.bar(monthly_dept_data, x='شهر', y=['Closed', 'Opened'], 
                               title="📈 المقارنة الشهرية للحالات (مغلقة vs مفتوحة)",
                               barmode='group',
                               color_discrete_map={'Closed': '#00b894', 'Opened': '#ff7675'})
            
            fig_monthly.update_traces(texttemplate='%{y}', textposition='inside')
